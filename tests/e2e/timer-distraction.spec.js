...
//
// Also covers the multi-distraction-per-session flow: two distractions
// captured within the same focus session must both appear in Inventory
// and share the same parent-session back-link (SC-006).
...
test("Distraction: two distractions in one session both appear in Inventory with shared parent ref (SC-006)", async ({
  ...
  await expect(
    page.locator("#inventory-distractions-list .inventory-parentref-clickable")
  ).toHaveCount(2);
});
