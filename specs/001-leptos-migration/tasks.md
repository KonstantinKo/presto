...
- [ ] T025 [T:GREEN] [US3] Mirror `BridgeError` in `src-tauri/src/lib.rs` with the identical serde derive (the Tauri-side test from T023 passes AS A CONSEQUENCE of this implementation, not as a freshly-introduced assertion) — done-signal: `cargo test -p presto_lib bridge_error_serde_roundtrip` returns 0.
