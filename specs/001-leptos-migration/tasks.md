...
- [x] T245 [US4] Verify `baseline-cap` fail-closed by intentionally re-capturing 3 baselines on a throwaway branch — done-signal: CI exits 1 with the documented error message; throwaway branch is then deleted.

  Verified on 2026-05-10. Created `throwaway/baseline-cap-dry-run` from `001-leptos-migration` HEAD `ab10c7b`, modified 3 baseline PNGs (`auth-modal`, `calendar`, `timer`) with appended bytes to register as content changes, ran `BASELINE_BASE_REF=origin/main bash scripts/check-baseline-cap.sh` → **exit 1** with message `ERROR: 3 baseline PNGs changed (max 2 per PR; see Constitution Principle IV and specs/001-leptos-migration/plan.md §CI gates).` (each offending file listed). Reverted one baseline → re-ran gate → **exit 0** (`2 of 2 baseline changes`), confirming the cap=2 boundary. Worktree and branch removed via `git worktree remove --force` + `git branch -D`.
- [x] T246 [US1] Wire engine `web-sys` grep gate from plan.md §CI gates into `.agentex.yml` `qa.lint` — done-signal: clean HEAD's lint stage exits 0.
- [x] T247 [US1] Verify engine `web-sys` grep gate fail-closed by introducing a temporary `web_sys::` reference under `src/src/engine/` on a throwaway branch — done-signal: lint stage exits 1 with the documented error message; throwaway branch deleted.

  Verified on 2026-05-10. Created `throwaway/engine-purity-dry-run` from `001-leptos-migration` HEAD `ab10c7b`, appended `use web_sys::window;` (with `#[allow(unused_imports, dead_code)]` so the ref doesn't fail the build for unrelated reasons) to `src/src/engine/clock.rs`, ran `bash scripts/check-engine-purity.sh` → **exit 1** with message `ERROR: engine module references web-sys (Principle I — The Timer Is Sacred).` plus the file:line of the offender. Worktree and branch removed via `git worktree remove --force` + `git branch -D`.
- [x] T248 [US2] Update pre-commit hook (husky-equivalent) to scan both `Cargo.lock` (vs. workspace `Cargo.toml`) and `tests/e2e/package-lock.json` (vs. `tests/e2e/package.json`) for drift per Principle IX — done-signal: a deliberate `Cargo.toml`-without-`Cargo.lock` change in a throwaway branch is rejected by the hook.

  Implemented as `scripts/check-lockfile-drift.sh` (dual-mode: pre-commit on staged set, CI on `LOCKFILE_BASE_REF...HEAD`) plus `.githooks/pre-commit` (committed; runs lockfile-drift + engine-purity before each commit) and `scripts/install-git-hooks.sh` (sets `core.hooksPath = .githooks`; husky is not present post-Phase-6 since the root npm scope is gone). Verified on 2026-05-10: created `throwaway/lockfile-dry-run`, modified `Cargo.toml` without staging `Cargo.lock`, attempted commit → hook exit 1 with `ERROR: Cargo.toml changed without Cargo.lock (Principle IX — lockfile discipline).` and `git commit` returned non-zero (no commit appended). Worktree and branch removed. The same script is wired into `.agentex.yml` `qa.lint` (CI mode) and `.github/workflows/ci.yml` (CI mode); `.github/workflows/ci.yml` is also realigned with post-Phase-6 reality (workspace-frozen Cargo, e2e under tests/e2e/, root-level `npm ci` removed, baseline-cap + engine-purity + lockfile-drift gate steps added).
