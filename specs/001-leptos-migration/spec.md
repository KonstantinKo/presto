...
- **FR-013**: Closed domains in the post-cutover frontend (timer mode, session type, status bar display, manager state — each manager: auth, session, settings, navigation, tag, team, update) MUST be Rust enums (sum types). String-typed or open-enum representations of these domains are forbidden in the new code. (The "sound notification variant" item was dropped from this list as a spec-revision correction: the on-disk shape is `notifications.sound_notifications: bool` — a single on/off toggle, not a closed variant set, so the closed-domain promise does not apply.)
...
- **FR-017**: This feature MUST NOT add new user-facing features. New settings, new screens, new shortcuts, new commands beyond what is required by the migration itself (e.g., a `track_event` command wrapping `tauri-plugin-aptabase`, an `export_sessions_xlsx` command replacing the JS `xlsx` dependency) are out of scope and MUST be deferred to follow-up issues.
- **FR-018**: This feature MUST NOT switch the analytics or auth provider. Aptabase remains the analytics provider, with the same opt-in toggle and the existing default — Principle II's default-off posture is enforced for fresh installs, and issue #26 tracks aligning the runtime default with the constitution. Supabase remains the optional auth provider, with guest mode first-class per Principle II.
...
- **A2 — Backend frozen**: All Rust code under `src-tauri/` is unchanged scope-wise. New Tauri commands solely *to enable* the migration (e.g., `track_event` wrapping `tauri-plugin-aptabase`, an `export_sessions_xlsx` command replacing the JS `xlsx` dependency) are in-scope; all other backend changes are deferred follow-ups per FR-019.
