...
- `pause(clock)` (lines 664-683). **Changed**: FR-013a requires `pause(clock)` to settle wall-clock elapsed into `current_session_elapsed_secs` before clearing the start anchor / timestamp state, so that `complete` and `abort` invoked from Paused observe the true elapsed at pause time.
